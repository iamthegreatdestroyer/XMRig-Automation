...
if (-not $xmrigProcess) {
    Write-Host "  ⚠️  WARNING: XMRig is not running!" -ForegroundColor Yellow
...
if (-not (Test-Path "C:\XMRig\profit-switcher-v2.ps1")) {
    Write-Host "  ❌ ERROR: Profit Switcher not found at C:\XMRig\profit-switcher-v2.ps1" -ForegroundColor Red
    Write-Host ""
    ...
    Write-Host "  ✅ Profit Switcher copied!" -ForegroundColor Green
    Write-Host ""
}
...
if (-not (Test-Path "C:\XMRig\configs\config-xmr.json")) {
    Write-Host "  ⚠️  WARNING: config-xmr.json not found" -ForegroundColor Yellow
    $configsExist = $false
}
if (-not (Test-Path "C:\XMRig\configs\config-rtm.json")) {
    Write-Host "  ⚠️  WARNING: config-rtm.json not found" -ForegroundColor Yellow
    $configsExist = $false
}
if (-not (Test-Path "C:\XMRig\configs\config-vrsc.json")) {
    Write-Host "  ⚠️  WARNING: config-vrsc.json not found" -ForegroundColor Yellow
    $configsExist = $false
}

...
class ThermalPredictor:
    ...
    def predict(self, seconds_ahead: Optional[int] = None) -> float:
        ...
        t0 = self._timestamps[0]
        current_t = self._timestamps[-1] - t0
        future_t = current_t + horizon
...
if __name__ == "__main__":
    ...
    # Example 1: Basic ThermalPredictor usage
    ...
    base_temp = 60.0
    for i in range(20):
        temp = base_temp + i * 0.8 + random.uniform(-0.5, 0.5)
        predictor.add_reading(temp)
    ...
    print(f"Should throttle: {predictor.should_throttle()}")
    
    # Example 2: ThermalController with mock functions
    print("\n=== ThermalController Demo ===")
    
    temps = iter([65, 68, 72, 75, 78, 80, 82, 78, 74, 70, 65, 62])
    
    def mock_temp_reader():
        return next(temps, 60)
    
    def mock_thread_adjuster(count):
        print(f"  -> Adjusting threads to {count}")
    
    controller = ThermalController(
        config=ThermalConfig(max_temp=85, target_temp=75),
    ...
    for i in range(12):
        ...
        print(f"Step {i+1}: {status['current_temp']:.0f}°C -> "
              f"pred:{status['predicted_temp']:.0f}°C | "
              f"threads:{status['threads']} | {status['action']}")

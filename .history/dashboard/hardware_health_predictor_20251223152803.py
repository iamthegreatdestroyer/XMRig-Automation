...
class MetricsCollector:
    ...
    def get_history_array(self, normalize: bool = True) -> np.ndarray:
        """Get feature matrix from history."""
        with self._lock:
            if not self._history:
                return np.array([])
            raw = np.array([s.to_array() for s in self._history])
            if normalize and self._scaler and len(raw) >= 10:
    ...
    def get_latest_sample(self) -> Optional[MetricsSample]:
...
class HardwareHealthPredictor:
    ...
    SCORE_THRESHOLDS = {"healthy": 55, "degraded": 30}  # Below 30 = critical
    ...
    def train(self, collector: MetricsCollector) -> bool:
        ...
        with self._lock:
            X = collector.get_history_array(normalize=False)  # Raw data for IF
            ...
            # Calibrate: get score distribution from training data
            train_scores = self._model.decision_function(X)
            self._score_offset = np.percentile(train_scores, 50)
            self._score_range = np.percentile(train_scores, 95) - np.percentile(train_scores, 5)
    ...
    def _compute_feature_importance(self, X: np.ndarray, names: List[str]) -> None:
        ...
        if not self._trained or len(X) < 50:
            return
    ...
    def predict_health(self, sample: MetricsSample, collector: MetricsCollector) -> HealthReport:
        ...
        with self._lock:
            ...
            X = sample.to_array().reshape(1, -1)  # Raw features
            raw_score = self._model.decision_function(X)[0]
            
            # Normalize and map to 0-100 using sigmoid for smooth gradation
            normalized = (raw_score - self._score_offset) / (self._score_range + 0.001)
            # Sigmoid mapping: more gradual transition through degraded zone
            health_score = 100 / (1 + np.exp(-normalized * 3))

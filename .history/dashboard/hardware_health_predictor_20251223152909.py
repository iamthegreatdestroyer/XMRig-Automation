...
class HardwareHealthPredictor:
    ...
    def predict_health(self, sample: MetricsSample, collector: MetricsCollector) -> HealthReport:
        ...
        with self._lock:
            ...
            # Map raw score to 0-100 health score
            # IF scores: positive = inlier (healthy), negative = outlier (anomaly)
            # Typical range: -0.5 (severe anomaly) to +0.3 (very normal)
            health_score = max(0, min(100, 60 + raw_score * 150))

...
function New-ScriptHashes {
    ...
    # Find all PowerShell scripts (excluding history, test, backup directories)
    $scripts = Get-ChildItem -Path $projectRoot -Filter "*.ps1" -Recurse -ErrorAction SilentlyContinue |
    Where-Object { 
        $exclude = $false
        # Check excluded directories
        foreach ($dir in $Script:IntegrityConfig.ExcludeDirs) {
...
# Export only when loaded as module (suppress error when dot-sourced)
